<template>
  <div>
    ...
    <svg :viewBox="
      ...
      @mouseup="touchend($event)" @mouseleave="touchend($event)" @wheel="mouseWheel($event)">
      <!-- 関連趣味に関する描画のループ -->
      <g v-for="(nodes, tierIndex) in splittedRelativeNodes" :key="'t-' + tierIndex">
      ...
      <g v-for="(node, index) in randomTags" :key="index">
        <nuxt-link :to="'/' + node.id + '/graph?from=' + targetNode.id">
          <circle :r="nodeParam.RADIUS" :cx="x + (index + 1) * 250" :cy="y + 300" class="randomTags"></circle>
          <text text-anchor="middle" dominant-baseline="central" style="font-size: 24px; fill: #111111">
