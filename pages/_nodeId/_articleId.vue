    ...
    <div class="container is-max-desktop">
      ...
      <div class="level mb-0">
        ...
        <div class="buttons is-right">
          <button class="button is-primary" :class="{ 'is-light': isZbzbPushed == false }" @click="zbzbButton()">
      ...
      <div class="tags">
        <nuxt-link v-for="(tag, index) in tags" :key="tag.id + index" :to="'/' + tag.id + '/graph'"
          class="tag is-warning is-rounded">
          {{ tag.name }}({{ tag.articlesCount }})
        </nuxt-link>
      </div>
      <mavon-editor v-model="content" language="ja" :subfield="false" :editable="false" :toolbars-flag="false"
        :box-shadow="false" default-open="preview" preview-background="#fff" />
        ...
        <div class="columns">
          ...
          <div class="level">
            <div class="column has-text-right">
              <button :disabled="isNoComment" type="button" class="button" :class="{ 'is-primary': !isNoComment }"
                @click="submit">
                コメントする
              </button>
            </div>
